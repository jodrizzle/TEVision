...
import qualified OpenCV.ImgProc.MiscImgTransform.ColorCodes as C 
...
process::[FilePath]->IO ()
process files = do 
        --imgOrig  <- CV.imdecode CV.ImreadUnchanged <$> B.readFile ("../data/Input/"++ (head files))      
        imgGS    <- CV.imdecode CV.ImreadGrayscale <$> B.readFile ("../data/"++ (head files))
        --let imgGS = CV.exceptError $ CV.cvtColor C.bgr C.gray imgOrig
      --tighten matrix type constraints to work with canny and blur-----------------------------------
        ...
        let canniedImg = cannyImg $ gaussianBlurImg formImg 3--laplace formImg--(medianBlurImg formImg 3)--(read (blur !! 1   )))
            
        --showImage "Original" imgGS
       -- showImage "Cannied Image" canniedImg
...
--laplace = CV.exceptError . CV.laplacian Nothing Nothing Nothing Nothing        
...
saveDetectedObjects::Int->(V.Vector (V.Vector CV.Point2i))-> M.Mat ('CV.S '[ 'CV.D, 'CV.D]) ('CV.S 1) ('CV.S Word8)->String->IO ()
saveDetectedObjects iter contours  imgGS fname
    ...
    | otherwise                   = do
        ...
        B.writeFile ("../data/Output/"++fnameNoExt++"_"++show iter++".tif") $ CV.exceptError $ CV.imencode CV.OutputTiff $ threshBinary $ cropImg uprightImg (V2 0 0) dims
        putStrLn $ "Wrote to file: ../data/Output/"++fnameNoExt++"_"++show iter++".tif"      
        when (V.length contours > 1) $ saveDetectedObjects (iter+1) (V.tail contours)  imgGS fname
